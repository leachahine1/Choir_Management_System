...
class Account extends MX_Controller {
     ...
     * 		http://rehearsalple.com/index.php/account
     * 	- or -  
     * 		http://rehearsalple.com/index.php/account/<method_name>
     */
    ...
    //This function will show Choir_members own due and pay
    public function due_pay() {
        $user = $this->ion_auth->user()->row();
        $user_id = $user->id;
        $Choir_member_id = $this->common->Choir_member_id($user_id);
        $data['slips'] = $this->accountmodel->own_slips($Choir_member_id);
        $this->load->view('temp/header');
        $this->load->view('due_pay', $data);
        $this->load->view('temp/footer');
    }

    //This function will load all Choir_members trangections slips
    ...
    //Show invioce or Choir_members tranjection slips details
    public function view_invoice() {
        $slipId = $this->input->get('sid', TRUE);
        $data['invoice'] = $this->accountmodel->invoice($slipId);
        $data['schoolName'] = $this->common->schoolName();
        $data['currency'] = $this->common->currencyChoir();
        $this->load->view('temp/header');
        $this->load->view('invoice', $data);
        $this->load->view('temp/footer');
    }

    //This function will pay Choir_members fees
    public function fee_pay() {
        if ($this->input->post('submit', TRUE)) {
            ...
            if ($this->db->update('slip', $slip_data)) {
                $data['message'] = '<div class="alert alert-success alert-dismissable">
								<button aria-hidden="true" data-dismiss="alert" class="close" type="button"></button>
								<strong>WOW!</strong> Your donation was successfully processed.
							</div>';
                $data['slips'] = $this->accountmodel->stud_payment();
                $this->load->view('temp/header');
                $this->load->view('allSlips', $data);
    ...
    //This function will edit Choir_member payment information
            ...
            echo '<div class="form-group">
                    <label class="col-md-3 control-label"></label>
                        ...
                        <div class="alert alert-danger">
                            <strong>' . lang('tea_info') . ':</strong> ' . lang('teac_1') . ' <strong>' . $Choir_memberId . '</strong>' . lang('teac_2') . '
                    </div></div></div>';
        } else {
            echo '<div class="row"><div class="col-md-offset-2 col-md-7 stuInfoIdBox">
                    <div class="col-md-8">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="Choir" value="' . $this->common->Choir_title($query->Choir_id) . '" readonly>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-2">
            ...
            if ($pre_balence >= $total_amount) {
                ...
                if ($this->db->insert('salary', $salary)) {
                    ...
                    if ($entry_info == 'no_entry') {
                        ...
                        $this->db->insert('donation', $inco_data);
                    } else {
                        $inco_data = array(
                            'date' => $this->db->escape_like_str(strtotime(date('d-m-Y'))),
                            'acco_id' => $this->db->escape_like_str(2),
                        ...
                        $this->db->update('donation', $inco_data);
                    }
                }
                $satSalaryInfo = array(
    ...
    //This function will make donation
        ...
        if ($this->input->post('expense', TRUE)) {
            ...
            if ($pre_balence >= $amount) {
                ...
                if ($entry_info == 'no_entry') {
                    ...
                    if ($this->db->insert('donation', $inco_data)) {
                    ...
                    $row_id = $entry_info[0]['id'];
                    $this->db->where('id', $row_id);
                    if ($this->db->update('donation', $inco_data)) {
                        $data['message'] = '<div class="alert alert-block alert-success fade in">
                                            <button data-dismiss="alert" class="close" type="button"></button>
                                            <h4 class="alert-heading">' . lang('success') . ' </h4> ' . lang('teac_8') . '
                                    </div>';
    ...
    public function exp_list_da_ra() {
        ...
        $query = $this->db->query("SELECT * FROM donation WHERE date >='$rngstrt' AND date <= '$rngfin' AND category='Expense'");
        $i = 1;
    ...
    public function inc_list_da_ra() {
        $rngstrt = strtotime($this->input->post('rngstrt', TRUE));
        $rngfin = strtotime($this->input->post('rngfin', TRUE));
        $query = $this->db->query("SELECT * FROM donation WHERE date >='$rngstrt' AND date <= '$rngfin' AND category='Income'");
        $i = 1;
        foreach ($query->result_array() as $row) {
            echo '<tr>
                    <td>
    ...
    //This function will make Choir_members month end slip by auto calculation
    public function end_stu_calcu() {
        $Choir = $this->accountmodel->all_Choir();
        foreach ($Choir as $row) {
            $Choir_id = $row['id'];
            $m_t_fee = $this->accountmodel->total_fee($Choir_id);
            if (!empty($m_t_fee)) {
                ...
                $all_Choir_member = $this->accountmodel->all_Choir_members($Choir_id);
                foreach ($all_Choir_member as $row2) {
                    $Choir_member_id = $row2['Choir_member_id'];
                    $dues = $this->accountmodel->dues($Choir_member_id);
                    if ($dues != 0) {
                        $total = $amount + $dues;
                    } else {
                        $total = $amount;
                    }
                    $advance = $this->accountmodel->advance($Choir_member_id);
                    $status = 'Unpaid';
                    $paid = 0;
                    $balanec = 0;
                    if ($advance != 0) {
                    ...
                    $data = array(
                        ...
                        'Choir_member_id' => $this->db->escape_like_str($Choir_member_id),
                ...
                $this->db->update('Choir', $Choir_com);
            }
        }
    }
