...
class Configarationmodel extends CI_Model {
    ...
    //This function will return class rehearsal term
    public function checkChoirFee($a) {
        $query = $this->db->query("SELECT id FROM set_fees WHERE Choir_title = '$a'")->row();
        if (empty($query)) {
            $rowId = 'empty';
            return $rowId;
        } else {
    ...
    public function getChoirFee($id) {
    ...
    //This function will return Choir_member item fee
