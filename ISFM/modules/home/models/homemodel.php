...
class HomeModel extends CI_Model {
    ...
    public function presentEmploy() {
        ...
        $query = $this->db->query("SELECT id FROM section_leader_attendance WHERE date='$date' AND present_or_absent='Present'");
    ...
    public function absentEmploy() {
        $data = array();
        $date = strtotime(date('d-m-Y'));
        $query = $this->db->query("SELECT id FROM section_leader_attendance WHERE date='$date' AND present_or_absent='Absent'");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }return count($data);
    }
    ...
    public function atten_chart() {
        $data = array();
        $query = $this->db->query("SELECT Choir_title,attendance_percentices_daily FROM Choir");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }
        return $data;
